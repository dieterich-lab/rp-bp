...
def create_all_figures(config, args):
    ...
    samples = [(name, sample_name_map[name], is_replicate) for name in sample_names]
    ...
    conditions = [
        (name, condition_name_map[name], is_replicate) for name in replicate_names
    ]

    all_names = samples + conditions
    parallel.apply_parallel_iter(
        all_names, args.num_cpus, _create_figures, config, args, progress_bar=True
    )
...
def main():
    ...
    parser.add_argument(
        "-t", "--tmp", help="A location for temporary files", default=None
    )
    ...
    with open(tex_file, "w") as out:
        ...
        for sample_name in sample_names:
            ...
            for is_grouped, is_chisq in it:
                ...
                orf_length_line_graph = (
                    filenames.get_orf_length_distribution_line_graph(
                        config["riboseq_data"],
                        sample_name,
                        length=lengths,
                        offset=offsets,
