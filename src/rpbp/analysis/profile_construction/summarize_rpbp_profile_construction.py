...
def collect_lengths_and_offsets(sample, config, is_unique, note, args):
    ...
    for length in range(min_read_length, max_read_length + 1):
        ...
        if max_bf_var is not None:
            if (
                length_row["highest_peak_bf_mean"] <= min_bf_mean
            ) or length_row["highest_peak_bf_var"] >= max_bf_var:
    ...
    data = {'sample': sample, 'length': lengths, 
            'offset': offsets, 'status': statuses}
...
def create_fastqc_reports(sample_data, config, note, is_unique, args):
    ...
    all_infiles = [raw_data, without_adapters, with_rrna,
                   without_rrna, genome_bam, unique_bam]    
    all_fastqc_reports = [raw_data_fastqc, without_adapters_fastqc,
                    with_rrna_fastqc, without_rrna_fastqc,
                    genome_bam_fastqc, unique_bam_fastqc]
    all_paths = [raw_data_fastqc_path, without_adapters_fastqc_path,
                 with_rrna_fastqc_path, without_rrna_fastqc_path,
                 without_rrna_mapping_fastqc_path, without_rrna_mapping_fastqc_path]
...
def main():
    ...
    # TODO MultiQC
    parser.add_argument("-c", "--create-fastqc-reports", help="""If this flag 
    ...
    all_length_distributions = parallel.apply_iter_simple(
        samples,
        collect_all_read_length_distributions,
        config,
        note,
        args
    )
    ...
    all_length_distributions.loc[all_length_distributions.index.str.contains("unique"), "is_unique"] = "True"
    # adjust name - the function called gets the name from the file name
    all_length_distributions.index = all_length_distributions.index.str.replace("-unique", "")
    if note is not None:
        repl = ".{}".format(note)
        all_length_distributions.index = all_length_distributions.index.str.replace(repl, "")
    ...
    pandas_utils.write_df(all_length_distributions, 
                          summary_file, 
                          index=True, 
                          sep=',', 
                          header=True, 
    ...
    pandas_utils.write_df(all_lengths_and_offsets, 
                          summary_file, 
                          index=False, 
                          sep=',', 
                          header=True, 
    ...
    pandas_utils.write_df(frame_counts, 
                          summary_file, 
                          index=False, 
                          sep=',', 
                          header=True, 
