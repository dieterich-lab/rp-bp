...
commands = {"riboseq": r"\textsc{ribo-seq}\xspace"}
...
def create_fastqc_reports(name_data, config, args):
    ...
    with_rrna = filenames.get_with_rrna_fastq(config["riboseq_data"], name, note=note)
...
def create_read_filtering_plots(config_file, config, args):
    ...
    cmd = "get-all-read-filtering-counts {} {} {} {} {}".format(
        config_file, read_filtering_counts, overwrite_str, cpus_str, logging_str
    )
...
def main():
    ...
    with open(tex_file, "w") as out:
        ...
        latex.section(
            out, "Read length distributions", label=length_distribution_section_label
        )
        ...
        for name in sample_names:
            ...
            if os.path.exists(unique_read_length_distribution_image):
                latex.write_graphics(
                    out, unique_read_length_distribution_image, width=0.45
                )
            else:
        ...
        for name in sample_names:
            ...
            for length in range(min_read_length, max_read_length + 1):
                ...
                if (
                    length_row["highest_peak_profile_sum"]
                    < args.min_visualization_count
                ):
                ...
                if args.show_read_length_bfs:
                    ...
                    bayes_factor_image = (
                        filenames.get_metagene_profile_bayes_factor_image(
                            config["riboseq_data"],
                            name,
                            image_type=args.image_type,
                            is_unique=is_unique,
