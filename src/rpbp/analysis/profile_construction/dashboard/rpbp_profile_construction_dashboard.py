...
def parse_args():
    parser = argparse.ArgumentParser(description="""Launch a Dash app for quality 
...
def get_profiles_bars(samples, start_upstream_window, start_downstream_window,
                      end_upstream_window, end_downstream_window):
    ...
    for sample in samples:
        ...
        metagene_profiles = pd.read_csv(filenames.get_metagene_profiles(config['riboseq_data'], 
            sample_name, is_unique=is_unique, note=config.get('note', None)))
        ...
        for length, offset in zip(lengths, offsets):
            ...
            start_counts, end_counts =  get_window_counts(metagene_profile, int(offset),
                                                          start_upstream_window, start_downstream_window,
                                                          end_upstream_window, end_downstream_window)
...
def get_profiles_lines(samples, start_upstream_window, start_downstream_window,
                      end_upstream_window, end_downstream_window):
...
def metagene_plot_lines(metagene_profile_start, metagene_profile_end,
                  start_upstream_window, start_downstream_window,
                  end_upstream_window, end_downstream_window, 
...
start_upstream = config.get(
    ...
    metagene_options["metagene_start_upstream"]
)
...
# *** app components
option_unique = [
    dict(label="All mapped reads", value="False")
]
if is_unique:
    option_unique.append(
        dict(label="Uniquely mapped reads", value="True")
    )
...
radio_stacked_reads = dcc.RadioItems(
    ...
    options=[
        dict(label="All reads", 
             title="Show the number of reads after each step", 
             value=0),
        dict(label="Exclude ribosomal/poor quality", 
...
filename = filenames.get_riboseq_read_filtering_counts(
    config["riboseq_data"], project_name, sub_folder=sub_folder.as_posix(), note=config_note
    )
read_filtering_counts = pd.read_csv(filename)
read_filtering_counts["Sample"] = read_filtering_counts["sample"].apply(lambda x: sample_name_map[x])
...
read_length_distributions["sample"] = read_length_distributions["sample"].apply(lambda x: sample_name_map[x])
...
lengths_and_offsets["sample"] = lengths_and_offsets["sample"].apply(lambda x: sample_name_map[x])
...
frame_counts = pd.melt(frame_counts, id_vars='sample', value_vars=["frame", "frame+1", "frame+2"], var_name="Frame")
frame_counts["p_site"] = frame_counts.groupby(['sample'])['value'].transform(lambda x: x/x.sum())*100
frame_map = {"frame": "In-frame", 
             "frame+1": "In-frame + 1",
             "frame+2": "In-frame + 2"}
                    ...
                    [   
                        html.Div(
                            [
                                html.Div(
                                    [
                                        html.H2("""1. Stacked bars and funnel charts showing 
                                ...
                                html.Div(
                                    [
                                        ...
                                        html.Br(),
                                        html.Br(),
                                        html.Label("""Hint: Hover over chart to see proportions.""",
                                            style={"font-style": "italic", "vertical-align": "bottom"}
                                        ),
                                    ],
                                    className="box",
                                    style={"width": "40%"},
                        ...
                        html.Div(
                            [
                                html.Div(
                                    [   html.H2("""2. Per sample read length distribution and 
                                        metagene profiles"""
                                        ),
                                        html.Div(
                                            [
                                                html.Label("Select one sample",
                                                        style={"margin": "10px"},
                                                ),
                                                html.Div(
                                                    [
                                        ...
                                        html.Label("""Hint: Hover over bars to see the metagene
                        ...
                        html.Div(
                            [
                                html.Div(
                                    [
                                        html.H2("""3. Periodicity for all selected samples"""
                                        ),
                                        html.Br(),
...
def stacked_reads(selected, zoom):
    ...
    alignment_diff_counts = get_diff_counts(
        read_filtering_counts[stack_cts_order]
    )
...
def update_available_samples(selected):
    options = [
        dict(label=val, value=val) for val in selected.split(",")
    ]
...
@app.callback([Output('read_length_metagene_bars_fig', 'src'),
...
def read_length_metagene_bars(hoverData, sample):
    ...
    metagene_profile = metagene_profiles[m_length]
    start_counts, end_counts = get_window_counts(metagene_profile, 
                                                 0, # no offset 
    ...
    # remove ticks
    labels = ["" if item.get_text() != "0" else "TIS" for item in axes[0].get_xticklabels()]
    axes[0].set_xticklabels(labels)
    labels = ["" if item.get_text() != "0" else "TTS" for item in axes[1].get_xticklabels()]
...
@app.callback(
    Output("stacked_frames_fig", "figure"),
    Input("drop_samples", "value")
)
def stacked_frames(selected):
    ...
    fig = px.bar(
        ...
        color = "Frame",
        labels={'p_site':'% P-sites (ORF profiles)'}).update_layout({
            'plot_bgcolor': 'rgba(0,0,0,0)',
            'paper_bgcolor': 'rgba(0,0,0,0)'
    }).update_layout(legend_traceorder="reversed")
...
def all_metagenes(window_upstream, window_downstream, 
                  step, plot, selected):
