...
def main():
    ...
    logging.info(msg)
    bf_coverage = parallel.apply_parallel(
        rpbp_peptide_matches, args.num_cpus, get_orf_coverage, progress_bar=True
    )
    bf_coverage = pd.DataFrame(bf_coverage)

    msg = "Calculating Rp-chi coverage"
    logging.info(msg)
    chisq_coverage = parallel.apply_parallel(
        rpchi_peptide_matches, args.num_cpus, get_orf_coverage, progress_bar=True
    )
    ...
    bf_covered_hist, b = np.histogram(
        bf_coverage["coverage"], bins=hist_bins, range=hist_range, density=True
    )

    chisq_covered_hist, b = np.histogram(
        chisq_coverage["coverage"], bins=hist_bins, range=hist_range, density=True
    )
